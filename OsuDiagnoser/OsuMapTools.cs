...
namespace OsuDiagnoser
{
	public static class OsuMapTools
	{
		...
		public static (FileInfo[][] maps, FileInfo[][] sbs) ScanForMaps(DirectoryInfo osuFolder)
		{
			...
			var maps = sets.Select(s => s.EnumerateFiles().Where(f => f.Extension == ".osu").ToArray()).ToArray();
...
");
		...
		public static ((string? audioFile, string[] sbFiles)[][] mapDeps, (string? audioFile, string[] sbFiles)[][]
			sbDeps) GetDeps(FileInfo[][] maps, FileInfo[][] sbs)
		{
			Console.Write("Reading files required by maps and storyboards... ");
			var sw      = Stopwatch.StartNew();
			var mapDeps = MultiThreadTool.BatchTask(maps, set => set.Select(GetMapDependencies).ToArray(), 8);
			//var mapDeps = maps.Select(set => set.Select(GetMapDependencies).ToArray()).ToArray();
			var sbDeps = MultiThreadTool.BatchTask(sbs, set => set.Select(GetMapDependencies).ToArray(), 8);
